...
public class ChatHistoryService {
    ...
    public JSONObject getChatsByUserId(int senderId) {
        ...
        if(!chatList.isEmpty()) {
            response.put("senderId" , chatList.get(0).getSenders().getUserId());
            response.put("senderName" , chatList.get(0).getSenders().getFirstName());
        }
        JSONArray receivers = new JSONArray();
        for (ChatHistory chats: chatList) {
            JSONObject receiverObj = new JSONObject();
            receiverObj.put("receiverId" , chats.getReceivers().getUserId());
            receiverObj.put("receiverName" , chats.getReceivers().getFirstName());
    ...
    public JSONObject getConversation(int user1, int user2) {
        ...
        for (ChatHistory chat : chatList) {
            ...
            messageObj.put("senderName" , chat.getSenders().getFirstName());
